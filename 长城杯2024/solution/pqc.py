...
class Dilithium:
    ...
    def sign_message(self, message: bytes) -> bytes:
        ...
        self.dilithium_lib.pqcrystals_dilithium2_ref(
            sm_buf, ctypes.byref(smlen_buf), m_buf, MLEN, self.sk_buf
        )
    ...
    def verify_sign(self, message: bytes, signature: bytes) -> bool:
        ...
        result = self.dilithium_lib.pqcrystals_dilithium2_ref_open(
            msg_buf, ctypes.byref(msg_len), sm_buf, len(signature), self.pk_buf
        )
...
class Task(socketserver.BaseRequestHandler):
    ...
    def get_sk(self):
        ...
        s1 = polyvec_t()
        s2 = polyvec_t()
        self.dilithium.dilithium_lib.pqcrystals_dilithium2_ref_unpack_sk(
            rho, tr, key, t0, s1, s2, self.dilithium.sk_buf
        )
    ...
    def handle(self):
        ...
        answers = sum(
            [
                self.dilithium.verify_sign(
                    long_to_bytes(self.m),
                    bytes.fromhex(sinature.zfill(len(signature) + len(signature) % 2)),
                )
                for sinature in signatures
            ]
        )
        if answers == num:
